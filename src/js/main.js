...
const player02 = document.querySelector('#feiticeira-escarlate');
var exportCharacterCont = 0;


let auxSelecterCharacter = false

// Vai percorrer todos os li de characters
...
characters.forEach((personagem) => {
    ...
    personagem.addEventListener('mouseenter', () => {
        ...
        if (!auxSelecterCharacter) {
            ...
            characters.forEach((player02) => {
                if (!(player02.getAttribute('class') === 'character second-player_seleted')) {
                    player02.classList.add('second-player_seleted');
                }
            })
...
let btnConf = document.querySelector('#conf').addEventListener('click', () => {
    ...
    if (exportCharacterCont === 2) {
        ...
        // Mandar os characters selecionados para o game 
